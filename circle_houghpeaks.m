...
if isempty(params.nhoodxy) && isempty(params.nhoodr)
    ...
    np = 0;
    while true
        [r, c, k, v] = max3(h); %coge el máximo global
        %fprintf('valor de v: %f\n', v);
        % stop if peak height below threshold
        
        if v < params.threshold || v == 0 %si el máximo está por debajo del th, no hay ninguno, termina.
            break;
        end
        np = np + 1; %aumenta el número de picos encontrados.
        peaks(:, np) = [c; r; radii(k)]; %guarda su información en el vector de picos
        % stop if done enough peaks
        if np == maxpks %si el número de picos es el máximo, sale.
            break;
        end
        % suppress this peak
